...
export class InventoryReportComponent implements OnInit {
  ...
  async loadReport() {
    ...
    const action = { type: 'POST', target: 'reports/view_inventory_reports' };
    ...
    // result = await result['data']['resp'].map((item) => {
    //   return {
    //     ...item,
    //     resident_name:
    //       item.residents.last_name + ' ' + item.residents.first_name,
    //     first_user_name:
    //       item.firstUsers.last_name + ' ' + item.firstUsers.first_name,
    //     second_user_name:
    //       item.secondUsers.last_name + ' ' + item.secondUsers.first_name,
    //     date: item.ts_date_created,
    //     drug_name:
    //       item.drugs.name +
    //       ' ' +
    //       item.drugs.suffix_name +
    //       ' ' +
    //       '( ' +
    //       item.drugs.non_suffix_name +
    //       ' )' +
    //       ' ' +
    //       item.drugs.strength.toFixed(2) +
    //       ' ' +
    //       item.drugs.unit.split('/')[0] +
    //       ' ' +
    //       item.drugs.dosage_form,
    //   };
    // });
  ...
  async inventoryReportSubmit(report, isValid) {
    ...
    if (isValid) {
      ...
      console.log('inventory report payload----->', payload);
      // return
