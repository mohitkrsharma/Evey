...
@Component({
  selector: 'app-carebytime',
  templateUrl: './carebytime.component.html',
  styleUrls: ['./carebytime.component.scss']
})
export class CarebytimeComponent implements OnInit {
  ...
  async submit(event) {
    ...
    if (this.value.length) {
      ...
      const action = { type: 'GET', target: 'reports/carebytime' };
      const payload = { 'range': arr, pagination: this.pagiPayload };
  ...
  async exportXlsx() {
    let action = {
      type: 'GET',
      target: 'reports/carebytime/export'
    }
