...
@Component({
  selector: 'app-custom-monthly',
  templateUrl: './custommonthly.component.html',
  styleUrls: ['./custommonthly.component.scss']
})
export class CustomMonthlyComponent implements OnInit {
  ...
  constructor(private _apiService: ApiService,
    ...
    public _dialogRef: MatDialogRef<CustomMonthlyComponent>,
  ...
  getSchedulePerformingTimeString(schedule) {
    ...
    if (repeat == 'custom_weekly') {
      ...
      let finalString = `Care will occur every ${schedule.repeat_tenure > 1 ? schedule.repeat_tenure : ''} ${schedule.repeat_tenure == 1 ? 'year' : 'years'} in the month of ${month[0].key} on the ${repeat[0].key} of ${day[0].name ? day[0].name.charAt(0).toUpperCase() + day[0].name.slice(1) : day[1].name.charAt(0).toUpperCase() + day[0].name.slice(1)}`
      return finalString;
    }
  }
